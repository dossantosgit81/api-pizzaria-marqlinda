...
@DirtiesContext
public class OrdersCommonUserIntegrationsTest {
    ...
    @Test
    public void shouldReturnOnlyOrdersDoneUsersCommon1_WhenSearchForTheTokenUser1(){
        ...
        //Results
        int id = response.jsonPath().getInt("content[0].id");
        int totalElements = response.jsonPath().getInt("totalElements");
        String email = response.jsonPath().getString("content[0].user.email");

        //Asserts
        assertEquals(1, id);
        assertEquals("travolta@test.com", email);
